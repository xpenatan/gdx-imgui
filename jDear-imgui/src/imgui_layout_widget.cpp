...
void ImGui::BeginCollapseLayout(const char* title, bool* isOpen, float sizeX, float sizeY, ImGuiCollapseLayoutOptions options)
{
      ImGuiContext& g = *GImGui;
      ImGuiWindow* window = g.CurrentWindow;
      ...
      if (!options.customFrameLayout)
          ImGui::EndCollapseFrameLayout(options);
      else
          ImGui::SameLine();
}

void ImGui::EndCollapseFrameLayout(ImGuiCollapseLayoutOptions& options)
{
    ImGuiContext& g = *GImGui;
    float bk = g.Style.ItemSpacing.y;
    ...
    ImGui::BeginLayout("content", ImLayout::MATCH_PARENT, ImLayout::WRAP_PARENT, options.paddingLeft, options.paddingRight, options.paddingTop, options.paddingBottom);
