...
public class ImGuiNative {
	...
	*/
	...
	@Deprecated
	public static native void getDrawData(int index, DrawData jDrawData, Buffer indexBuffer, Buffer vertexBuffer, Buffer cmdBuffer) /*-{ }-*/; /*
		ImDrawData * drawData = ImGui::GetDrawData();
		...
		if(drawData != NULL) {
			...
			jfieldID idxCountID = env->GetFieldID(jDrawDataClass, "idxCount", "I");
			jfieldID vtxCountID = env->GetFieldID(jDrawDataClass, "vtxCount", "I");
			jfieldID cmdCountID = env->GetFieldID(jDrawDataClass, "cmdCount", "I");
			...
			if(index < cmdListsCount) {
				ImDrawList & drawList = *drawLists[index];
				ImVector<ImDrawCmd> & imDrawCmdList = drawList.CmdBuffer;
				ImVector<ImDrawIdx> & idxBuffer = drawList.IdxBuffer;
				ImVector<ImDrawVert> & vtxBuffer = drawList.VtxBuffer;

				env->SetIntField (jDrawData, idxCountID, idxBuffer.Size);
				env->SetIntField (jDrawData, vtxCountID, vtxBuffer.Size);
				env->SetIntField (jDrawData, cmdCountID, imDrawCmdList.Size);
				...
				for(int j = 0; j < vtxBuffer.Size; j++) {
					...
					int byteIndex = j * (4 + colorSize);

					float color = 0;
					memcpy(&color, &v.col, 4); // move unsigned int color to float

					vertexArrayDest[byteIndex] = posX;
					vertexArrayDest[byteIndex + 1] = posY;
					vertexArrayDest[byteIndex + 2] = uvX;
					vertexArrayDest[byteIndex + 3] = uvY;
					vertexArrayDest[byteIndex + 4] = color;
				...
				int indexBytesSize = idxBuffer.Size * 2;
				memcpy(indexArrayDest, indexArraySource, indexBytesSize);
				...
				for (int cmd_i = 0; cmd_i < imDrawCmdList.Size; cmd_i++) {
					...
					memcpy((cmdArrayDest + (cmd_i * imDrawCmdSize)), tempArray, imDrawCmdSize * 4);
					bytesOffset = bytesOffset + imDrawCmdSize;
				}
			}
		}
